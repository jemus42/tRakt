...
#' @param genres `character(n)`: Genre [slugs](https://trakt.docs.apiary.io/#reference/genres).
#' Multiple values are allowed and will be concatenated.
#' @param languages `character(n)`: Two-digit language code. Also see [`languages`] for
#' table of available languages.
#' @param countries `character(n)`: Two-character country code. See [`countries`].
#' @param runtimes `character | integer`: Numeric range in minutes, e.g. `30-90`. Can also be an integer
#' vector of length two which will be coerced appropriately.
#' @param ratings `character | integer`:  Numeric range between `0` and `100`. Can also be an integer
#' vector of length two which will be coerced appropriately.
#' @param certifications `character(n)`: Certification like `pg-13`.
...
#' @param networks Network name like `HBO`. See [`networks`] for a list of known networks.
#' @param status `character(n)`: The status of the shows. One of `returning series`,
