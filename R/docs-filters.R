...
#' @param years `character | integer`: 4-digit year (`2010`) **or** range, e.g. `"2010-2020"`.
#'   Can also be an integer vector of length two which will be coerced appropriately.
#' @param genres `character(n)`: Genre  slug(s). See [`trakt_genres`] for a table of genres.
#'   Multiple values are allowed and will be concatenated.
#' @param languages `character(n)`: Two-digit language code(s). Also see [`trakt_languages`] for
#'   table of available languages.
#' @param countries `character(n)`: Two-character country code(s). See [`trakt_countries`].
#' @param runtimes `character | integer`: Integer range in minutes, e.g. `30-90`. Can also
#'   be an integer vector of length two which will be coerced appropriately.
#' @param ratings `character | integer`:  Integer range between `0` and `100`. Can also be
...
#'   Multiple values are allowed. Use [`trakt_certifications`] for reference. Note that there
#'   are different certifications for shows and movies.
#' @param networks `character(n)`: (Shows only) Network name like `HBO`. See [`trakt_networks`]
