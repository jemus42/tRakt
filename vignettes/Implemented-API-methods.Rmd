...
format_api_table <- function(xdf) {
  ...
  mutate(
    ...
    authentication = if_else(!is.na(authentication), "Required", "-")
  ) %>%
  select(
    -url, 
    Endpoint = endpoint, 
...
Authenticated methods (there's only a few) are indicated as such, whereas a "-" in that column indicates authentication is optional.
