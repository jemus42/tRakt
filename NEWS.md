...
- New filter builders `filters_movies()`, `filters_shows()`, and
  ...
  `shows_trending()`, ...) and `search_query()`. This centralizes and documents
  filtering, adds the
...
- Passing filters as individual arguments to the dynamic-list functions and
  `search_query()` (`genres`, `years`, `networks`, ...) is soft-deprecated in
  favour of the
